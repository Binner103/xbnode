...
import { getPosts, createPost, updatePost, deletePost, createPostTag, postHasTag, deletePostTag, getPostTotalCount} from './post.service';
...
export const storePostTag = async (
    ...
    if (!tag) {
        try {
            ...
            tag = { id: data.insertId } // 将新建的标签ID交给tag
        } catch (error) {
            return next(error);
        }
    }
